...
export function AddResourceDialog({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Ensure identification data is properly formatted before saving
    const finalData: ResourceFormData = {
      ...formData,
      // Make sure identification data is included as a proper object
      identificationData: formData.identificationData || {
        owner: "",
        contactInfo: "",
        accessType: "public",
        createdBy: ""
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:text-white"
                    required
                  >
                    <option value="" disabled>Select a category</option>
                    {categories.map((category) => (
                      <option key={category.id} value={category.id} className="bg-background text-foreground dark:bg-gray-800">
                        {category.name}
                      </option>
                    ))}
                  </select>
